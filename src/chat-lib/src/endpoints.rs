...
pub fn create_message(conn: &mut PgConnection, new_message: &NewMessage) -> QueryResult<Message> {
...
fn new_convo(conn: &mut PgConnection, title: Option<&str>) -> QueryResult<Conversation> {
    diesel::insert_into(conversations::table)
        .values(crate::models::NewConvoInternal { id: None, title })
        .get_result(conn)
}
...
pub fn get_conversation_users(
    ...
    conversation_users
        .filter(
            user_conv_id.eq(convs
                .filter(conv_id.eq(conversation_id.parse::<i32>().unwrap_or(-1)))
